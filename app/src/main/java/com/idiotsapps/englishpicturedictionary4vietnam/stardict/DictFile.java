package com.idiotsapps.englishpicturedictionary4vietnam.stardict; /**
...
public class DictFile {
    ...
    private boolean isEnableStream = false;

    public DictFile(FileInputStream fileInputStream, FileOutputStream fileOutputStream) {
    ...
    public String getWordData(long offset, long size) {
        ...
        try {
            if (isEnableStream) {
                dt = new DataInputStream(new BufferedInputStream(this.fileInputStream));
            } else {
            ...
            Log.d("Read file name '" + strFileName + "'", ex.toString());
        } finally {
            if (dt != null) {
                try {
                    dt.close();
                } catch (Exception ex) {
                    Log.e("Closing DataInputStream", ex.toString());
                }
            }
        }
        return strMeaning;
    ...
    public long addData(String strMeaning) {
        ...
        try {
            if(isEnableStream) {
                dt = new DataOutputStream(this.fileOutputStream);
            }else{
            ...
            Log.e("Read file name '" + strFileName + "'", ex.toString());
        } finally {
            if (dt != null) {
                try {
                    dt.close();
                } catch (Exception ex) {
                    Log.e("Closing DataOutputStream", ex.toString());
                }
            }
        }
        return fileSize;
